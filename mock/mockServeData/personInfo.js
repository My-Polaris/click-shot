...
function queryUserInfo(myId,userId){
  ...
  //查看该用户的粉丝列表是否有我
  let index = userInfo.fansUserList.indexOf(myId);
  let focus = index>=0?true:false;
  ...
  return {userInfo,pubPostList,pubWorkList,focus};
}
...
export default {
  ...
  getUserInfo: config => {
    ...
    let { userInfo,pubPostList,pubWorkList,focus } = queryUserInfo(myId,userId);
    return {
      meta: {
        status: 200,
        msg: '获取成功',
